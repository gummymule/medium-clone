<div class="flex bg-white border border-gray-200 rounded-lg shadow-sm mb-8">
    <div class="p-5 flex-1">
        <div class="flex flex-row mb-4 text-sm text-gray-500">
            Published by
            <a href="{{ route('profile.show', $post->user->username) }}"
                class="ml-1 text-gray-900 hover:underline"
            >
                {{ $post->user->username }}
            </a>
        </div>
    ...
    <a href="{{ route('post.show', [
        ...
        <img 
            class="w-48 h-full max-h-64 object-cover rounded-r-lg" 
            src="{{ $post->imageUrl() ?? Storage::url($post->image) }}" 
            alt="" 
        />
    </a>
</div>